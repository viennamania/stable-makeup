  ...
  /*
{
  ...
  */
  ...
  const storecode = "ycvqgqgg"; // 타이틀 가맹점
  ...
  try {
    ...
    if (users && users.length > 0) {
      ...
      for (let i = 0; i < users.length; i++) {
        ...
        const response = await fetch("https://dubai-telegram.vercel.app/api/telegram/sendMessageByUseridAndStorecodeBananaBot", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({
            center: "oneclick_banana_bot",
            userid: userid,
            storecode: storecode,
